...
export default function MainView() {
	const {data, isLoading} = useQuery(["weather", "now"], getCurrentWeather);

	return (
		<main className="w-full p-4">

			{isLoading ? <Loader /> : (<>
				<CityName city={data.name} country={data.sys.country} />
				<section className="w-full bg-blue-500 rounded-lg my-4 overflow-hidden">
					<div className="w-full flex items-center justify-center flex-col p-4">
						...
						<div className="flex items-center flex-col gap-2">
							<h2 className="text-white text-2xl capitalize">{data.weather[0].description}</h2>
							<p className="text-slate-300 text-xs">{formatDate(Date.now())}</p>
						</div>
						<h1 className="text-8xl text-white font-bold my-6">{formatTemp(data.main.temp)}°</h1>
					</div>
					...
					<div className="grid grid-cols-2 grid-rows-2 h-[200px] border-collapse">
						<div className="w-full h-full border-y-[1px] border-t-white p-3 flex items-center gap-2">
							...
							<aside>
								...
								<p className="text-white text-xs">{data.wind.speed}km/h</p>
							</aside>
						</div>
						<div className="w-full h-full border-[1px] border-r-0 border-white p-3 flex items-center gap-2">
							...
							<aside>
								...
								<p className="text-white text-xs">{formatTemp(data.main.feels_like)}°</p>
							</aside>
						</div>
						<div className="w-full h-full p-3 flex items-center gap-2">
							...
							<aside>
								...
								<p className="text-white text-xs">{data.main.humidity}%</p>
							</aside>
						</div>
						<div className="w-full h-full border-l-[1px] border-l-white p-3 flex items-center gap-2">
							...
							<aside>
								...
								<p className="text-white text-xs">{data.main.pressure} mbar</p>
							</aside>
						</div>
					</div>
				</section>
